...
function prepareSql(checkpointId: boolean) {
  const sql = `
  SELECT
    thread_id,
    checkpoint_ns,
...
export class CloudflareD1Saver extends BaseCheckpointSaver {
  ...
  async getTuple(config: RunnableConfig): Promise<CheckpointTuple | undefined> {
    ...
    const sql = prepareSql(!!checkpoint_id);
  ...
  async putWrites(
    ...
    await this.db.batch(rows.map((row) => this.db.prepare(sql).bind(...row)));
  }
  ...
  protected async migratePendingSends(
    ...
    const row = await this.db
      ...
      .bind(threadId, parentCheckpointId)
      .first<{ pending_sends: string }>();

    const { pending_sends } = row as { pending_sends: string };
